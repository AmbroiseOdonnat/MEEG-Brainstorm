...
class Loader():
    ...
    def pad_loader(self, data, labels, transforms=None, shuffle=False):
    ...
    def LOPO_dataloader(self):
        ...
        train_subject_ids = np.delete(
                                 subject_ids,
                                 np.where(subject_ids == self.subject_LOPO))
        ...
        if self.data_augment == "offline":
            ...
            for subject_id in train_subject_ids:
                for i in range(len(data[subject_id])):
                    ...
                    n = len(labels[subject_id][i])
                    if n_spikes/n < 0.2:
                        n_no_spike_to_remove = ((n-n_spikes) - 4*n_spikes)
                        pos_no_spike = np.where(labels[subject_id][i] == 0)
                        no_spike_to_remove = np.random.choice(
                                                    pos_no_spike[0],
                                                    size=n_no_spike_to_remove,
                ...
                if n != 0:
                    ...
                    if new_data != []:
                        ...
                        data[subject_id][i] = np.concatenate(
                                                [data[subject_id][i],
                                                 new_data])
                        labels[subject_id][i] = np.concatenate(
                                                [labels[subject_id][i],
                                                 [1 for _ in range(n_new_data)]
                                                 ])
        ...
        for id in train_subject_ids:
            for n_sess in range(len(data[id])):
                for n_trial in range(len(data[id][n_sess])):
                    ...
                    chan = self.annotated_channels[id][n_sess]
                    if self.single_channel:
        ...
        test_data = []
        test_labels = []
        for n_sess in range(len(data[self.subject_LOPO])):
            for n_trial in range(len(data[self.subject_LOPO][n_sess])):
    ...
    def train_val_test_dataloader(self):
        ...
        """
        dataset = []
        subject_ids = np.asarray(list(self.data.keys()))

        for id in subject_ids:
            for n_sess in range(len(self.data[id])):
                for n_trial in range(len(self.data[id][n_sess])):
                    x = np.expand_dims(self.data[id][n_sess][n_trial], axis=0)
                    y = self.labels[id][n_sess][n_trial]
                    chan = self.annotated_channels[id][n_sess]
    ...
    def load(self):
        if self.subject_LOPO:
            return self.LOPO_dataloader()
        else:
            return self.train_val_test_dataloader()
